(function ($) {
	...
	document.addEventListener('DOMContentLoaded', () => {
		...
		const transitionBlocks = document.querySelectorAll(".block");

		// Prevent default link behavior and handle custom transitions
		function handlePageTransition(event) {
			event.preventDefault();
			const link = event.currentTarget;
			const targetUrl = link.getAttribute("href");

			// Skip internal links, hash links, and current page
			if (!targetUrl ||
			...
			// Trigger exit transition before navigation
			animateExitTransition().then(() => {
				window.location.href = targetUrl;
			});
		}

		// Add transition event listeners to all links
		...
		function revealPageEntrance() {
			gsap.set(transitionBlocks, {
				scaleY: 1,
				visibility: "visible"
			});

			return new Promise((resolve) => {
				gsap.to(transitionBlocks, {
					scaleY: 0,
					duration: 1,
					stagger: {
						each: 0.1,
		...
		// Exit transition before page change
		...
		// Initialize page entrance transition
		revealPageEntrance().then(() => {
			gsap.set(transitionBlocks, { visibility: "hidden" });
		});
	});
